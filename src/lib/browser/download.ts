...
export async function downloadText(filename: string, content: string, mimeType = 'text/plain;charset=utf-8', saveAs = true): Promise<void> {
  const dataUrl = `data:${mimeType};base64,${btoa(unescape(encodeURIComponent(content)))}`;
  await browser.downloads.download({ url: dataUrl, filename, saveAs });
}
